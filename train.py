...
def read_data_and_preprocess(file_path):
    ...
    if mileage1 is None or price is None:
        print("Training data could not be loaded.")
        return None, None
    return mileage1, price,mileage_min, mileage_max

def normalizeAndError(file_path):
    mileage, price, mileage_min, mileage_max = read_data_and_preprocess(file_path)
    ...
    return theta0, theta1,mileage_min, mileage_max
...
def save_model_parameters(theta0, theta1, mileage_min, mileage_max, file_path='model_parameters.txt'):
    try:
        with open(file_path, 'w') as f:
            f.write(f"{theta0},{theta1},{mileage_min},{mileage_max}")
        print(f"Model parameters saved to {file_path}")
    except Exception as e:
        print("An error occurred while saving model parameters:", e)
...
def main():
    if len(sys.argv) != 2 or not sys.argv[1].endswith('.csv'):
        ...
        return
    try:
        theta0, theta1, mileage_min, mileage_max = normalizeAndError(sys.argv[1])
        save_model_parameters(theta0, theta1, mileage_min, mileage_max)
    except Exception as e:
